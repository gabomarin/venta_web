				...
				if( !isset($_SESSION['mail']) || $_SESSION['tipo'] == 2 ){//un visitante y el encargado de ventas puede crear un nuevo usuario
					...
					if( isNombre($_REQUEST['nombre']) && isMail($_REQUEST['mail']) && isPass($_REQUEST['pass']) && isDireccion($_REQUEST['direccion']) && 
						...
						if ( is_object($usuario) )
						{
							...
							$smarty->assign('usuario_nombre', $usuario->nombre);
							$smarty->assign('usuario_mail', $usuario->mail);
							
							
							$var= $smarty->fetch('email_registro.tpl');
							ob_start();
							echo $var;
							$msg= ob_get_clean();
							
							
							$enlace= '<a class="btn btn-success" href="index.php?modulo=estandar&action=login">AQUI</a>';
